# Customer Behavior Genie Operations
...
```sql
-- Mode A examples
GRANT SELECT ON TABLE juan_use1_catalog.retail.gold_customer_dim TO `user@example.com`;
GRANT SELECT ON TABLE juan_use1_catalog.retail.gold_sales_fact TO `user@example.com`;

-- Mode B examples
GRANT SELECT ON TABLE juan_use1_catalog.retail.customer_segmentation_mv TO `user@example.com`;
GRANT SELECT ON TABLE juan_use1_catalog.retail.customer_rfm_analysis_mv TO `user@example.com`;
```
...
SELECT * FROM juan_use1_catalog.retail.gold_customer_dim LIMIT 1;
SELECT * FROM juan_use1_catalog.retail.customer_segmentation_mv LIMIT 1;
```
...
- Ensure sample queries include representative customer behavior patterns.
...
### Poor segmentation/RFM results
- Confirm filtering logic (current customers, return handling, and date range).
- Add targeted sample queries for segmentation and RFM edge cases.
